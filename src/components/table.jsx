    ...
    <table className="overflow-x-scroll">
      ...
      {data.map(d => (
        <tr key={d.id} className="border-t text-center py-2">
          ...
          <td>{ d.uuid }</td>
          <td>{ d.balance }</td>
          <td><button onClick={e => flip(d.id, d.balance)} className="border py px-8 block rounded-sm transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 hover:text-orange-600 duration-300 ">flip</button></td>
        </tr>
      ))}
      </tbody>
    </table>
